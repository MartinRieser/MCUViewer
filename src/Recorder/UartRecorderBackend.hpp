#ifndef _UARTRECORDERBACKEND_HPP
#define _UARTRECORDERBACKEND_HPP
...
/**
 * @brief Recorder backend implementation for UART debug probe
 *
 * This backend provides both software and hardware recording modes:
 * - Software mode: Polling via readMemory() (like STLink/JLink)
 * - Hardware mode: Future implementation using target-side buffering
 *
 * Target sample rate: 100 Hz for software mode (similar to STLink).
 */
class UartRecorderBackend : public RecorderBackend
{
   public:
	...
	enum class Mode
	{
		SOFTWARE,    // Software polling via readMemory()
		HARDWARE     // Hardware buffering (future implementation)
	};

	/**
	 * @brief Construct UART recorder backend
	 * @param probe Shared pointer to UART debug probe instance
	 * @param mode Recording mode (software or hardware)
	 */
	explicit UartRecorderBackend(std::shared_ptr<UartDebugProbe> probe, Mode mode = Mode::SOFTWARE)
					   ...
					   std::vector<double>& values) override
	{
		...
		{
			lastError_ = "UART probe not connected";
			return false;
		}
		...
		// Software mode: read each variable individually
		...
		{
			// TODO: Implement hardware buffering mode
			// This will use target-side buffering with bulk transfers
			lastError_ = "Hardware recording mode not yet implemented";
			return false;
		}
	}
	 ...
	 * @return true if hardware mode is available (future feature)
	 */
	bool supportsHardwareRecording() const override
	{
		// Hardware mode will be implemented in future tasks
		return false;
	}
   ...
   private:
	...
	std::shared_ptr<UartDebugProbe> probe_;
	Mode mode_;
	std::string lastError_;
};

#endif // _UARTRECORDERBACKEND_HPP
