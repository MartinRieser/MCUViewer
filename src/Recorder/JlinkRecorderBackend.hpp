#ifndef _JLINKRECORDERBACKEND_HPP
#define _JLINKRECORDERBACKEND_HPP
...
/**
 * @brief Recorder backend implementation for JLink debug probe
 *
 * This backend provides software recording by polling JLink's readMemory()
 * method. Target sample rate: 1 kHz for multiple variables (10x faster than STLink).
 */
class JlinkRecorderBackend : public RecorderBackend
{
   public:
	/**
	 * @brief Construct JLink recorder backend
	 * @param probe Shared pointer to JLink debug probe instance
	 */
	explicit JlinkRecorderBackend(std::shared_ptr<JlinkDebugProbe> probe)
		: probe_(probe)
	{
	}
					   ...
					   std::vector<double>& values) override
	{
		...
		{
			lastError_ = "JLink probe not connected";
			return false;
		}
		...
		for (size_t i = 0; i < addresses.size(); i++)
		{
			...
			// Read memory - JLink's readMemory returns bool
	 ...
	 * @return false - JLink software recording only (HSS mode is separate)
	 */
   ...
   private:
	...
	std::shared_ptr<JlinkDebugProbe> probe_;
	std::string lastError_;
};

#endif // _JLINKRECORDERBACKEND_HPP
